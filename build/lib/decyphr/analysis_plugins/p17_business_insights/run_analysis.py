...
def analyze(ddf, analysis_results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "p04_advanced_outliers" in analysis_results:
        ...
        if "total_outliers" in outlier_data:
            count = outlier_data["total_outliers"]
            pct = outlier_data.get("percentage", 0)
            if count > 0:
                insights.append({
                    "category": "Risk & Quality",
                    "severity": "High" if pct > 5 else "Medium",
                    "insight": f"Detected {count} ({pct}%) potential anomalies/outliers in the dataset.",
                    "detail": "These records deviate significantly from the norm and may represent fraud, errors, or high-value edge cases."
                })
